...
from .models import User, Resume, Application, CompanyImage, Follow, JobPost, JobCategory, CompanyProfile, \
    CandidateProfile, Review
from .perms import IsReviewOwner
from .serializers import UserSerializer, ResumeSerializer, ApplicationSerializer, FollowSerializer, \
    CompanyImageSerializer, JobPostSerializer, JobCategorySerializer
...
class CompanyView(viewsets.ViewSet, generics.RetrieveAPIView):
    ...
    queryset = CompanyProfile.objects.filter(active=True).select_related('user').prefetch_related('images')
